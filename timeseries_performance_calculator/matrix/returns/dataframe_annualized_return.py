# dataframe_annualized_return.py

from .annualized_return_calculator import calculate_annualized_return_cagr, calculate_annualized_return_days
import pandas as pd

def calculate_cumreturn_of_timeseries(timeseries: pd.DataFrame)->float:
    price_i = timeseries.iloc[:, 0].iloc[0]
    price_f = timeseries.iloc[:, 0].iloc[-1]
    return (price_f / price_i - 1) * 100

def get_trading_days_of_timeseries(timeseries: pd.DataFrame)->int:
    return len(timeseries)

def calculate_annualized_return_of_timeseries(timeseries: pd.DataFrame, calculator)->float:
...
def calculate_cagr_of_timeseries(timeseries: pd.DataFrame)->float:
    return calculate_annualized_return_of_timeseries(timeseries, calculate_annualized_return_cagr)

def calculate_annualized_days_return_of_timeseries(timeseries: pd.DataFrame)->float:
